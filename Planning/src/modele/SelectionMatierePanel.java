...
public class SelectionMatierePanel extends JPanel{
	...
	public SelectionMatierePanel(){
		...
		for (Module module : listeModule) {
			liste.put(module, dao.findHeuresSessionModule(module, session).getNbreHeuresDisponibles());
			
		}
		
		for(Module clefs : liste.keySet()){
			JRadioButton bouton = new JRadioButton(clefs.getNomModule() + " : "+ liste.get(clefs) + " heures disponibles");
